...
func (aq *AIQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(ai.Table, ai.FieldID, selector),
...
// First returns the first AI entity from the query.
// Returns a *NotFoundError when no AI was found.
func (aq *AIQuery) First(ctx context.Context) (*AI, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{ai.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first AI ID from the query.
// Returns a *NotFoundError when no AI ID was found.
...
// Only returns a single AI entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one AI entity is found.
// Returns a *NotFoundError when no AI entities are found.
func (aq *AIQuery) Only(ctx context.Context) (*AI, error) {
	...
	case 0:
		return nil, &NotFoundError{ai.Label}
	default:
		return nil, &NotSingularError{ai.Label}
	}
}
...
// OnlyID is like Only, but returns the only AI ID in the query.
// Returns a *NotSingularError when more than one AI ID is found.
...
func (aq *AIQuery) All(ctx context.Context) ([]*AI, error) {
	...
	qr := querierAll[[]*AI, *AIQuery]()
	return withInterceptors[[]*AI](ctx, aq, qr, aq.inters)
}
...
func (aq *AIQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if err = aq.Select(ai.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (aq *AIQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, aq, querierCount[*AIQuery](), aq.inters)
}
...
// Clone returns a duplicate of the AIQuery builder, including all associated steps. It can be
...
func (aq *AIQuery) Clone() *AIQuery {
	...
	return &AIQuery{
		config:     aq.config,
		ctx:        aq.ctx.Clone(),
		order:      append([]ai.OrderOption{}, aq.order...),
		inters:     append([]Interceptor{}, aq.inters...),
		predicates: append([]predicate.AI{}, aq.predicates...),
...
func (aq *AIQuery) GroupBy(field string, fields ...string) *AIGroupBy {
	...
	grbuild := &AIGroupBy{build: aq}
	grbuild.flds = &aq.ctx.Fields
	grbuild.label = ai.Label
...
func (aq *AIQuery) Select(fields ...string) *AISelect {
	...
	sbuild := &AISelect{AIQuery: aq}
	sbuild.label = ai.Label
...
func (aq *AIQuery) prepareQuery(ctx context.Context) error {
...
func (aq *AIQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AI, error) {
	var (
		nodes       = []*AI{}
		withFKs     = aq.withFKs
		_spec       = aq.querySpec()
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, ai.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*AI).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &AI{config: aq.config}
	...
	if query := aq.withUser; query != nil {
		if err := aq.loadUser(ctx, query, nodes, nil,
			func(n *AI, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AIQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*AI, init func(*AI), assign func(*AI, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*AI)
	for i := range nodes {
		if nodes[i].user_ais == nil {
			continue
		}
		fk := *nodes[i].user_ais
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_ais" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AIQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := aq.querySpec()
...
func (aq *AIQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(ai.Table, ai.Columns, sqlgraph.NewFieldSpec(ai.FieldID, field.TypeInt))
	_spec.From = aq.sql
...
func (aq *AIQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(aq.driver.Dialect())
	t1 := builder.Table(ai.Table)
...
func (agb *AIGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*AIQuery, *AIGroupBy](ctx, agb.build, agb, agb.build.inters, v)
}

func (agb *AIGroupBy) sqlScan(ctx context.Context, root *AIQuery, v any) error {
...
func (as *AISelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*AIQuery, *AISelect](ctx, as.AIQuery, as, as.inters, v)
}

func (as *AISelect) sqlScan(ctx context.Context, root *AIQuery, v any) error {
