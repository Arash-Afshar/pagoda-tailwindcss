...
func (pq *ProductQuery) loadPrices(ctx context.Context, query *PriceQuery, nodes []*Product, init func(*Product), assign func(*Product, *Price)) error {
	...
	for _, n := range neighbors {
		fk := n.product_prices
		if fk == nil {
			return fmt.Errorf(`foreign-key "product_prices" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "product_prices" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
