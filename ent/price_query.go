...
func (pq *PriceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Price, error) {
	...
	if pq.withUser != nil || pq.withProduct != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withUser; query != nil {
		if err := pq.loadUser(ctx, query, nodes, nil,
			func(n *Price, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PriceQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Price, init func(*Price), assign func(*Price, *User)) error {
...
func (pq *PriceQuery) loadProduct(ctx context.Context, query *ProductQuery, nodes []*Price, init func(*Price), assign func(*Price, *Product)) error {
	...
	nodeids := make(map[int][]*Price)
	for i := range nodes {
		if nodes[i].product_prices == nil {
			continue
		}
		fk := *nodes[i].product_prices
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "product_prices" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
